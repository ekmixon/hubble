...
def _check_password_fields_not_empty(block_id, block_dict, extra_args):
    ...
    result = _execute_shell_command(
        'cat /etc/shadow | awk -F: \'($2 == "" ) { print $1 " does not have a password "}\'', python_shell=True
    )
...
def _check_core_dumps(block_id, block_dict, extra_args):
    ...
    Ensure core dumps are restricted
    """
    hard_core_dump_value = _execute_shell_command(
        "grep -R -E \"hard +core\" /etc/security/limits.conf /etc/security/limits.d/ | awk '{print $4}'",
        python_shell=True,
    ).strip()
...
def _check_all_users_home_directory(block_id, block_dict, extra_args=None):
    ...
    users_uids_dirs = _execute_shell_command(
        'cat /etc/passwd | awk -F: \'{ print $1 " " $3 " " $6 " " $7}\'', python_shell=True
    ).strip()
...
def _check_users_dot_files(block_id, block_dict, extra_args):
    ...
    users_dirs = _execute_shell_command(
        "cat /etc/passwd | egrep -v '(root|halt|sync|shutdown)' | awk -F: '($7 != \"/sbin/nologin\") {print $1\" \"$6}'",
        python_shell=True,
    ).strip()
...
def _mail_conf_check(block_id, block_dict, extra_args):
    ...
    mail_addresses = _execute_shell_command(
        "grep '^[[:blank:]]*inet_interfaces' /etc/postfix/main.cf | awk -F'=' '{print $2}'", python_shell=True
    ).strip()
