...
def execute(block_id, block_dict, extra_args=None):
    ...
    if secret is False:
        return runner_utils.prepare_negative_result_for_module(
            block_id, "registry value couldn't " "be fetched for reg_name {0}".format(reg_name)
        )
...
def get_failure_reason(block_id, block_dict, extra_args=None):
    ...
    :param extra_args:
        ...
        Example: {'chaining_args': {'result': "HKEY_LOCAL_MACHINE\Software\Policies\Microsoft\Windows\EventLog\Applica
                  'caller': 'Audit'}
    :return:
    """
    reg_name = runner_utils.get_param_for_module(block_id, block_dict, "name")
