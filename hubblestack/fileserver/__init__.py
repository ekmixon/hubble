...
from hubblestack.utils.args import get_function_argspec as _argspec
...
def reap_fileserver_cache_dir(cache_base, find_func):
    ...
    for saltenv in os.listdir(cache_base):
        ...
        for root, dirs, files in hubblestack.utils.path.os_walk(env_base):
...
def generate_mtime_map(opts, path_map):
    ...
    """
    file_map = {}
    for saltenv, path_list in iter(path_map.items()):
        for path in path_list:
            for directory, _, filenames in hubblestack.utils.path.os_walk(path):
        ...
        while attempt < 11:
            try:
                ...
                if 0 <= age < opts.get('fileserver_list_cache_time', 20):
                    ...
                    with hubblestack.utils.files.fopen(list_cache, 'rb') as fp_:
                        log.debug(
                            "Returning file list from cache: age=%s cache_time=%s %s",
                            age, opts.get('fileserver_list_cache_time', 20), list_cache
                        )
                        return hubblestack.utils.data.decode(serial.load(fp_).get(form, [])), False, False
...
class Fileserver(object):
    ...
    def __init__(self, opts):
        self.opts = opts
        self.servers = hubblestack.loader.fileserver(opts, opts['fileserver_backend'])
    ...
    def find_file(self, path, saltenv, back=None):
        ...
        saltenv = hubblestack.utils.stringutils.to_unicode(saltenv)
        back = self.backends(back)
        kwargs = {}
        fnd = {'path': '',
               'rel': ''}
    ...
    def __file_hash_and_stat(self, load):
        ...
        fnd = self.find_file(hubblestack.utils.stringutils.to_unicode(load['path']),
                load['saltenv'])
        if not fnd.get('back'):
            return '', None
    ...
    def symlink_list(self, load):
        ...
        if prefix != '':
            ret = dict([
                (x, y) for x, y in iter(ret.items()) if x.startswith(prefix)
            ])
        return ret
